...
void search(int len) {
  ...
  priority_queue<Node> q; /* don't let larger string overwrites small one */
  for(int i = 0; i < ns; i++)
    if(dp[i][len] != "-") {
  ...
  while(!q.empty()) {
    ...
    for(int i = 0; i < tr[x].size(); i++) {
      ...
      if(dp[empty][0] == "" && (dp[target][len] == "-" || s < dp[target][len]) /* don't let larger string overwrites small one */) {
        dp[target][len] = s;
